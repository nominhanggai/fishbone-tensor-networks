...
using whatever ``(SpinBoson model, MPS engine)`` classes are passed in.  The same
...
def run_multichannel_ic(SpinBoson, Mps, sigma_x, sigma_z, num_op, *, lbo=False,
    ...
    SpinBoson : class
        ...
        ``SpinBoson(pd, coup_mat=..., freq=..., temp=...)`` with ``.build(n=0)``
    ...
    with redirect_stdout(buf):          # the legacy engine is extremely chatty
        eth = SpinBoson(pd, coup_mat=coup_mat, freq=freq, temp=temp)
