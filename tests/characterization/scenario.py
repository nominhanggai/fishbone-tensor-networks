...
using whatever ``(BosonicBath model, MPS engine)`` classes are passed in.  The same
...
def run_multichannel_ic(BosonicBath, Mps, sigma_x, sigma_z, num_op, *, lbo=False,
    ...
    BosonicBath : class
        ...
        ``BosonicBath(pd, coup_mat=..., freq=..., temp=...)`` with ``.build(n=0)``
        and ``.get_u(t, dt, factor=...)``.
    Mps : class
    ...
    with redirect_stdout(buf):          # the legacy engine is extremely chatty
        eth = BosonicBath(pd, coup_mat=coup_mat, freq=freq, temp=temp)
