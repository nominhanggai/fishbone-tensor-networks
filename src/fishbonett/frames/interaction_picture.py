...
absorbed into time-dependent couplings ``d_n(t)`` (:meth:`BosonicBathIP.mode_couplings`).
...
* ``BosonicBath.run(method="tebd")`` -- two-site Trotter gates (:meth:`get_u`)
  applied by the swap network;
* ``BosonicBath.run(method="trotter-mpo")`` -- the same propagator written exactly
